pipeline {
    ...
    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'bhavana', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/bhavanasBS/devopsDocker.git", branch: 'main'
                }
            }
        }
        ...
        stage('Login to Docker Registry') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'bhavanaSb', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
